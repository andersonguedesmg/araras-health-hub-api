...
{
    public class GetReceivingsByPeriodQueryHandler : IRequestHandler<GetReceivingsByPeriodQuery, PagedResult<ReceivingListItemResponse>>
    {
        private readonly IReceivingRepository _receivingRepository;

        public GetReceivingsByPeriodQueryHandler(
        ...
        public async Task<PagedResult<ReceivingListItemResponse>> Handle(
            GetReceivingsByPeriodQuery request,
            CancellationToken cancellationToken)
        {
            IQueryable<Receiving> query = _receivingRepository
                ...
                .Where(x =>
                    x.ReceivingDate >= request.StartDate &&
                    x.ReceivingDate <= request.EndDate);
            ...
            return PagedResult<ReceivingListItemResponse>.Success(
                ...
                request.PageSize,
                totalCount,
                "Recebimentos por período listados com sucesso.");
        }
    }
}
